...
## Environment Bootstrap (required, run first)

Before following any of the workflow sections in this document, read and complete every step in the sibling document [`ENVIRONMENT_BOOTSTRAP.md`](ENVIRONMENT_BOOTSTRAP.md). It sets up the canonical plate-authority alias cache that downstream helpers in this skill require; workflows below will fail to normalize plate codes if the bootstrap has not run.

Do not skip the bootstrap - it must be run once per environment regardless of whether the current task appears to need cross-dataset code normalization.
